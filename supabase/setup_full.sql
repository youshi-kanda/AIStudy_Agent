...
-- Steps Table
CREATE TABLE steps (
  id TEXT PRIMARY KEY, -- "api-basics-01" etc.
  course_id TEXT REFERENCES courses(id) ON DELETE CASCADE,
  "order" INTEGER NOT NULL,
  title TEXT NOT NULL,
  learning_goal TEXT,
  content_md TEXT, -- Markdown Content
  quiz JSONB, -- Store quiz object as JSON
  "references" JSONB, -- Store references list as JSON
  created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL
);
...
-- RLS Policies
-- Courses & Steps are public readable
